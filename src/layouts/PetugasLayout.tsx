...
export const PetugasLayout = ({ children }: PetugasLayoutProps) => {
  const { user } = useAuth();
      ...
      <div className="max-w-7xl mx-auto flex pt-20 min-h-screen">
        <aside className="bg-white shadow w-72 ">
          ...
          <div className="mt-5 ">
            <SidebarButton
              icon={FaHome}
              isActive={pathname === "/petugas/dash"}
              label="Dashboard"
              onClick={() => {
                navigate({ to: "/petugas/dash" });
              }}
            />
            <SidebarButton
              icon={FaListCheck}
              isActive={pathname.startsWith("/petugas/inputkelas")}
              label="Input Kelas"
              onClick={() => {
                navigate({ to: "/petugas/inputkelas" });
              }}
            />
            <SidebarButton
              icon={FaClipboard}
              isActive={pathname.startsWith("/petugas/lihatkelas")}
              label="Lihat Kelas"
              onClick={() => {
                navigate({ to: "/petugas/lihatkelas" });
              }}
            />
            <SidebarButton
              icon={FaSignOutAlt}
